 ...
 * @description: 分页信息封装类(修改getCurrentShowPage方法)
 */
public class PageBean<T> {
    ...
    public List<Integer> getCurrentShowPage() {
        currentShowPage.clear(); // 清空列表

        int startPage = Math.max(currentPage - showPageSize / 2, 1); // 计算起始页码
        int endPage = Math.min(startPage + showPageSize - 1, totalPage); // 计算结束页码
